...
- [Алгоритм генерации кода (Base62)](#алгоритм-генерации-кода-base62)
- [Сервисный слой](#сервисный-слой)
- [Точка входа: main.go](#точка-входа-maingo)
...
- [ ] Как определять доменные модели в Go (struct vs class)
- [ ] Как создавать кастомные ошибки с `errors.New` и `errors.As`/`errors.Is`
- [ ] Почему интерфейсы в Go маленькие и определяются у потребителя
- [ ] Как работает неявная реализация интерфейсов (duck typing)
- [ ] Как собирать граф зависимостей вручную без DI-контейнера
- [ ] Зачем передавать `context.Context` первым аргументом
- [ ] Как запускать асинхронные операции через горутины в сервисном слое
- [ ] Как организовать graceful shutdown HTTP-сервера

---

[← Обзор проекта](./README.md) | [Хранилище →](./02_storage.md)
