...
- [PostgreSQL с pgx v5](#postgresql-с-pgx-v5)
- [Реализация URLRepository](#реализация-urlrepository)
- [Redis кэш](#redis-кэш)
- [Паттерн cache-aside](#паттерн-cache-aside)
- [Обработка ошибок хранилища](#обработка-ошибок-хранилища)
...
- [ ] Как создавать и настраивать пул соединений pgx
- [ ] Как писать CRUD операции с pgx без ORM
- [ ] Как конвертировать `pgx.ErrNoRows` в доменные ошибки
- [ ] Как использовать `RETURNING` для получения сгенерированных значений
- [ ] Как применять паттерн `defer tx.Rollback()` для транзакций
- [ ] Как работать с Redis через go-redis: Get/Set/Del
- [ ] Как обрабатывать кэш-промах (`redis.Nil`)
- [ ] Как реализовать паттерн cache-aside
- [ ] Почему асинхронные операции в горутинах требуют нового контекста
- [ ] Как выстраивать цепочки ошибок через `fmt.Errorf("%w", err)`

---

[← Доменная модель](./01_domain.md) | [HTTP слой →](./03_http.md)
