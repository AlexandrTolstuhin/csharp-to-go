...
- [Чек-лист раздела](#чек-лист-раздела)
...
- [ ] Как писать многостадийный Dockerfile для Go (scratch образ)
- [ ] Почему Go образ значительно меньше .NET образа
- [ ] Как настроить Docker Compose с healthcheck для зависимостей
- [ ] Как реализовать graceful shutdown через `signal.Notify` и `server.Shutdown`
- [ ] Как читать конфигурацию из переменных окружения
- [ ] Как реализовать health check с проверкой зависимостей
- [ ] Как использовать `log/slog` для структурированного логирования
- [ ] Какие настройки важны для production (timeouts, pool, security)
