# Синтаксис и базовые концепции (сравнение с C#)
...
#### Трёхиндексный срез (Full Slice Expression)

> 💡 **Для C# разработчиков**: В C# аналога нет. Ближайшее — `ArraySegment<T>` с явным заданием диапазона, но без контроля capacity.

Обычный срез `s[low:high]` наследует capacity базового массива. Это значит, что `append` к подслайсу может неожиданно перезаписать данные в исходном массиве:

```go
arr := [5]int{1, 2, 3, 4, 5}
s := arr[1:3]  // [2 3], len=2, cap=4 (до конца arr!)

s = append(s, 99) // есть место в cap — перезаписывает arr[3]!
fmt.Println(arr)  // [1 2 3 99 5] — неожиданное изменение!
```

**Трёхиндексный срез** `s[low:high:max]` ограничивает capacity явно:

```go
// Синтаксис: s[low:high:max], где cap = max - low
arr := [5]int{1, 2, 3, 4, 5}
s := arr[1:3:3]  // [2 3], len=2, cap=2 — capacity ограничен!

s = append(s, 99) // cap исчерпан → новая аллокация, arr не тронут
fmt.Println(arr)  // [1 2 3 4 5] — оригинал не изменён
```

**Ограничения**:
- Требуется: `0 <= low <= high <= max <= cap(исходного)`
- Работает с массивами и слайсами (не со строками)
- `max` нельзя опустить — только явное значение

**Практическое применение — изоляция подслайса**:

```go
// ❌ Опасно: подслайс делит capacity с оригиналом
func head(data []byte) []byte {
    return data[:3] // cap = cap(data), append может испортить data
}

// ✅ Безопасно: ограничиваем cap, append создаст новый массив
func head(data []byte) []byte {
    return data[:3:3] // len=3, cap=3 — изолирован от оригинала
}
```

| Выражение      | len | cap         | append меняет исходный? |
|----------------|-----|-------------|------------------------|
| `arr[1:3]`     | 2   | cap(arr)−1  | Да, если есть место    |
| `arr[1:3:3]`   | 2   | 2           | Нет — новая аллокация  |
| `arr[1:3:5]`   | 2   | 4           | Нет до 4 элементов     |
